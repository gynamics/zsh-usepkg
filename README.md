...
- `:depends` just like `:after`, but aborts if one of specified dependency is missing.
  - in other words, `:after` follows weak dependencies, `:depends` follows strong dependencies.
...
- `:preface` allows you evaluate given string before the package dependencies are loaded.
- `:init` allows you evaluate given string after the package dependencies are loaded but before the package is loaded.
